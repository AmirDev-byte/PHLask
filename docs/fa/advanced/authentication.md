...
احراز هویت یکی از مهم‌ترین بخش‌های هر برنامه وب است که به شما امکان می‌دهد هویت کاربران را تأیید کنید و دسترسی‌های مناسب
را به آن‌ها ارائه دهید. در این بخش با روش‌های مختلف پیاده‌سازی احراز هویت در فلسک‌پی‌اچ‌پی آشنا می‌شوید.
...
2. **ورود (Login)**: کاربر با استفاده از اطلاعات ارائه شده در مرحله ثبت‌نام (معمولاً نام کاربری/ایمیل و رمز عبور) وارد
   سیستم می‌شود.
3. **احراز هویت مستمر (Persistent Authentication)**: سیستم باید بتواند کاربر را در جلسات بعدی بدون نیاز به ورود مجدد
   شناسایی کند.
...
JWT یک استاندارد باز برای انتقال امن داده‌ها به صورت JSON است. این روش برای API‌ها بسیار مناسب است زیرا stateless (بدون
حالت) است:
...
همیشه از HTTPS برای انتقال داده‌های حساس استفاده کنید. اطلاعات احراز هویت (مانند رمز عبور یا توکن) هرگز نباید از طریق
اتصال رمزگذاری نشده منتقل شوند.
...
همچنین با توصیه‌های امنیتی مهم مانند هش کردن رمزهای عبور، استفاده از HTTPS، محافظت در برابر حملات CSRF و محدود کردن
تلاش‌های ورود آشنا شدید.

برای پیاده‌سازی احراز هویت امن و مقیاس‌پذیر، توصیه می‌شود از روش‌های استاندارد و کتابخانه‌های معتبر استفاده کنید و
همواره به‌روزرسانی‌های امنیتی را دنبال کنید.