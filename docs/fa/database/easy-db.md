...
کتابخانه `EasyDB` یک لایه انتزاعی ساده و سبک برای تعامل با پایگاه داده در فلسک‌پی‌اچ‌پی است. این کتابخانه برای کاربرانی طراحی شده که به دنبال یک روش ساده و مستقیم برای کار با پایگاه داده هستند، بدون نیاز به یادگیری ویژگی‌های پیچیده‌تر کلاس‌های `Connection` و `QueryBuilder`.
...
کتابخانه `EasyDB` ساده‌تر و با API محدودتر نسبت به `QueryBuilder` است. انتخاب بین این دو به نیازهای پروژه شما بستگی دارد:
