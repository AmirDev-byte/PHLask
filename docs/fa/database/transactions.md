...
> **نکته**: پشتیبانی از تراکنش‌های تو در تو به نوع پایگاه داده بستگی دارد. MySQL (InnoDB) از طریق savepoints از آن
...
> پیاده‌سازی نشده‌اند. برای استفاده از آنها، باید از شیء PDO استفاده کنید یا این متدها را به کلاس `Connection` اضافه کنید.
